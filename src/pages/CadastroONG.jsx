...
import imagemComplementar4 from "../assets/ONG2.jpg";
...
function CadastroONG() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.senhaONG !== formData.confirmarSenhaONG) {
            alert("As senhas não coincidem!");
            return;
        }

        try {
            const response = await fetch("http://localhost/backend/cadastro_ong.php", {
                method: "POST",
                headers: {
    ...
    return (
        <div className="flex w-screen h-screen bg-zinc-900 overflow-hidden">
            ...
            <motion.div
                ...
                style={{
                    backgroundImage: `url(${imagemComplementar4})`,
                    ...
                    >
                        ...
                        <motion.div variants={itemVariants}>
                            <label className="block mb-2 text-zinc-300">
                                ...
                                <select
                                    name="areaAtuacaoONG"
                                    value={formData.areaAtuacaoONG}
                                    onChange={handleChange}
                                    className="w-full p-3 border-b-2 border-zinc-700 bg-zinc-900/50 text-white mt-1 focus:outline-none focus:border-blue-500 focus:bg-zinc-800 transition-all duration-300"
                                    required
                                >
                                    <option className="text-zinc-200" value="">Selecione uma opção</option>
                                    <option className="text-zinc-200" value="Meio Ambiente">Meio Ambiente</option>
                                    <option className="text-zinc-200" value="Educação">Educação</option>
                                    <option className="text-zinc-200" value="Saúde">Saúde</option>
                                    <option className="text-zinc-200" value="Direitos Humanos">Direitos Humanos</option>
                                    <option className="text-zinc-200" value="Assistência Social">Assistência Social</option>
                                    <option className="text-zinc-200" value="Cultura">Cultura</option>
                                    <option className="text-zinc-200" value="Outros">Outros</option>
                                </select>
                            </label>
                        </motion.div>
                        ...
                        <motion.div variants={itemVariants}>
                            <label className="block mb-2 text-zinc-300">
                                ...
                                <input
                                    type="password"
                                    name="confirmarSenhaONG"
                                    value={formData.confirmarSenhaONG}
                                    onChange={handleChange}
                                    className="w-full p-3 border-b-2 border-zinc-700 bg-zinc-900/50 text-white mt-1 focus:outline-none focus:border-blue-500 focus:bg-zinc-800 transition-all duration-300"
                                    required
                                />
